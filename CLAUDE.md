...
- **AI vision scan** (bad-quality papers): [aiScan.js](src/engine/aiScan.js) `scanViaApi(images)` POSTs rendered page IMAGES (base64 JPEGs from `ocr.js` `renderPageImages`) to **[api/scan.js](api/scan.js)** — a Vercel function that calls the multimodal Gemini (`buildScanPrompt` in [aiPrompt.js](src/engine/aiPrompt.js), shared) to TRANSCRIBE the questions, returning clean text the client re-parses. Triggered by `extractText` **only when local OCR/text under-parses** (`< MIN_QUESTIONS`), for signed-in users, with its own per-user daily cap (`api_usage` kind=`scan`, `SCAN_DAILY_CAP`). Solves blurry scans / mangled OCR / OCR-garbled question numbers that local Tesseract can't. Inert (503 → falls back to local) until `GEMINI_API_KEY` is set.
