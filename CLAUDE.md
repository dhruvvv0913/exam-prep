...
node scripts/test-marks.mjs "<pdf>"                 # diagnose per-question marks extraction for one paper
node scripts/crop-probe.mjs / crop-match.mjs        # dump/score "View original" crop strips (writes gitignored scripts/_crops/)
...
**PYQ-LY** is a React + Vite SPA that helps college students find the important, repeated questions in past exam papers, grouped by concept and ranked by how often they recur.
...
1. `extractText.js` — pdfjs text layer; if empty/garbled, falls back to OCR, keeping whichever yields more questions. If even the best local read is **under-parsed** (`< MIN_QUESTIONS`) AND an `aiScan` callback is provided (signed-in), a final **AI vision scan** transcribes the page IMAGES via Gemini (see `aiScan.js`/`api/scan.js`) and is kept if it recovers more questions. Returns `{ text, method: "text" | "ocr" | "ai" }`. Falls back silently on any AI failure/quota.
...
4. `parsePaper.js` — pure functions: detect exam type/session/year/subject/code (tolerant of OCR garble), strip KIIT noise + model answers in solution PDFs (the answer separator may be an en/em-dash: "Solution – …"), split into question units (number + part; a `Q1.`/`Q2)` prefix and OCR-garbled leading numbers on an "Answer all/the following" stem are also tolerated — decent scans used to be rejected over them). The **number style** (`1.` vs `1)`, via `detectStyle`) and the **part-bracket style** (`(a)` vs `a)`, via `detectPartStyle`) are detected **independently per-paper**, because real papers MIX them — e.g. `1.`-numbered questions with `a)` sub-parts (which used to silently drop every sub-part, and Q1 with them). `detectPartStyle` defaults to the stricter `(a)` on a tie/no-parts (so `(a)` papers are unaffected) and flips to `a)` only when bare parts dominate. An inline run of markers on one line (`a) … b) … c) …`) is treated as MCQ options / a run-in list (kept as content), NOT new sub-parts — this stops answer-option lines from spawning stub questions or corrupting the part/number sequence. Section-header stubs ("Short Questions") are dropped. Handles the KIIT mid-sem + end-sem (type 1/2) formats. `assessSolutionSheet(text, questionCount)` still detects **answer keys / solution sheets** (an explicit "Marking Scheme/Model Answer/Answer Key" header, or per-question "Solution –"/"Ans:" lead-ins covering ≥50% of questions or ≥6 absolute) and `parsePaper` exposes it as `meta.solution`, BUT this is no longer surfaced to the user — by product decision, a PYQ uploaded WITH its solutions is analysed silently and the result shown like any other (the model answers are already stripped to question stems by `cleanText`, so grouping works either way). `meta.solution` is currently vestigial (kept + unit-tested for possible future use); the pipeline emits no warning.
...
**"View original" question crops** ([cropQuestion.js](src/engine/cropQuestion.js) browser-only I/O + [cropCore.js](src/engine/cropCore.js) pure matching, unit-tested — same split as cluster/clusterCore): each question row on the analysis screen can show the EXACT image strip of that question cropped from its uploaded source file (scans read back garbled and diagrams can't be shown as text; display-only — grouping still uses extracted text). Boundaries come from positions, not pixels: pdf.js text-item coordinates (born-digital) or Tesseract line bboxes via `ocr.js` `ocrLines()` (scans/images); a question's strip = its first line down to the next question/part marker (KIIT papers are single-column). `findRegion` matches a question to its line by a strict ≥3-consecutive-leading-word run (`startScore`); when that fails — common on scans/screenshots because the crop RE-extracts the source independently of the read used for analysis (OCR vs AI-scan divergence) — it falls back to the **whole source page** carrying most of the question's content words (order-free, OCR-fuzzy via edit-distance ≤ 1), returned as `approximate: true` and shown with a "showing the full source page" note; `null` (an error note) only when nothing overlaps. In-session uploads only (crops straight from the in-memory `File`s; saved/library subjects show text only). Dynamically imported on first open; cached per paper+question. Validate crop alignment with `scripts/crop-probe.mjs` / `crop-match.mjs` — but note Node's `@napi-rs/canvas` renders born-digital PDFs with garbled glyphs (embedded-font issue, harness-only artifact); verify born-digital render quality in the browser.

**Assignment questions** (optional): on landing, the one "Slides & assignments" zone marks each uploaded file `slide` (default) or `assignment` (`handouts` is now `[{ id, file, kind }]`; `App` derives `slideFiles`/`assignmentFiles`). `assignmentFiles` are parsed like papers but each question is tagged `assignment: true` (paperId "Assignment", `pIdx` 1000+), so `rank.js` MERGES them into the topic groups and **boosts** them: `enrichGroup` excludes assignment items from the `appears` exam-count, sets `inAssignment`, keeps such a topic out of "asked once" (`unique = appears<2 && !inAssignment`), and `byWeight` sorts assignment-flagged topics first. The UI badges them ("★ In assignment" on the card, a gold "Assignment" chip per question; "View original" is hidden for them — their files aren't in `sources.papers`). A bad assignment file is skipped silently (never triggers the rollback).

`pipeline.analyze(paperFiles, { onProgress, slideFiles, assignmentFiles, aiGroup, aiScan })` reads `slideFiles` (optional slide PDFs; each deck is read separately and labelled from its filename), extracts deck topics, and uses slide-anchored grouping when slides exist (else bottom-up). A corrupt/unreadable paper is collected into `skipped` (a placeholder keeps `papers[pIdx]` index-aligned). **By product decision the whole run then THROWS** rather than returning a partial result — if *any* paper couldn't be scanned (unreadable, or 0 questions read), `analyze` throws naming the paper(s), and the UI rolls back to the upload screen with that reason (kept files, dismissible banner) instead of silently dropping a paper. (`skipped` is still returned but is non-empty only on the throw path.) Returns `{ papers, groups, questionCount, paperCount, topicCount, skipped, aiError }` (`skipped` = `[{ name, reason }]` for papers that yielded nothing/were unreadable; `aiError` = null, or the reason signed-in AI grouping fell back to the in-browser embedding grouping, e.g. `"group api 429"`). `skipped` is surfaced (a banner on the analysis screen + a live `paper-skipped` note in `LoadingScreen`); `aiError`, when set, raises a results-screen toast ("AI grouping was busy…" / "Daily AI limit reached" on a 429). The analysis screen derives its two views with `summarize()` (**By importance**) and `byPpt()` (**By PPT**); the **admin review** ([ReviewScreen.jsx](src/screens/ReviewScreen.jsx), reached via "Edit groups") edits `groups` directly (it preserves each group's `deck`, including for split-off "new group"s) — merge/split/rename/delete via one "move question to → group / new group" primitive — and `onGroupsChange` persists the edits. This is the guaranteed-accuracy step over the imperfect auto-grouping.

Keep `parsePaper`, `textQuality`, `clusterCore`, `rank`, `slides`, `cropCore`, `aiPrompt`, `mergeContent` as pure/Node-safe modules (no DOM/pdfjs) so the `scripts/` harnesses and `npm test` can use them; browser-only bits live in `extractText`/`ocr`/`cluster` (the embedder)/`cropQuestion`.
...
- `loading` ([LoadingScreen.jsx](src/screens/LoadingScreen.jsx)) — runs the **real** `pipeline.analyze(papers, { slideFiles })` with live stage progress (not a fake timer) plus a feature tour (3s auto-advance, prev/next arrows) while the user waits, then → analysis.
...
The LLM pass clearly beats in-browser embedding grouping (correct chapter per question, no off-syllabus dumping) and, being admin-side, stays free + campus-safe for students.
...
- **AI vision scan** (bad-quality papers): [aiScan.js](src/engine/aiScan.js) `scanViaApi(images)` POSTs rendered page IMAGES (base64 JPEGs from `ocr.js` `renderPageImages`) to **[api/scan.js](api/scan.js)** — a Vercel function that calls the multimodal Gemini (`buildScanPrompt` in [aiPrompt.js](src/engine/aiPrompt.js), shared) to TRANSCRIBE the questions, returning clean text the client re-parses. Triggered by `extractText` for signed-in users whenever the paper was **SCANNED (the OCR path won) OR under-parsed** (`< MIN_QUESTIONS`) — born-digital papers with a clean text layer skip it. "Keep whichever finds more questions" means it can only improve the read. (Signed-in users are the product priority, so the vision read is applied generously to scans.) Has its own per-user daily cap (`api_usage` kind=`scan`, `SCAN_DAILY_CAP`). Solves blurry scans / mangled OCR / OCR-garbled question numbers that local Tesseract can't. Inert (503 → falls back to local) until `GEMINI_API_KEY` is set.
- `App` owns a `useAi` toggle (**defaults on**) and passes `aiGroup` + `aiScan` to `LoadingScreen` only when signed in + on; `pipeline.analyze(…, { aiGroup, aiScan })` uses them and **falls back to in-browser embedding grouping / local extraction on any failure** (offline, quota, not deployed). `LandingScreen` shows the AI toggle (signed-in) or a "Sign in for sharper AI grouping" nudge (anonymous — kept sign-in-gated to protect the key/cost). `/api/group` + `/api/scan` don't run under `vite dev` — use `vercel dev` to exercise them locally (otherwise they just fall back).
