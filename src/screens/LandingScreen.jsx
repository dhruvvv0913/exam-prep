  ...
  return (
    <div style={{ position: "relative", flex: 1, minHeight: 0, overflowY: "auto" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 880, margin: "0 auto", padding: isMobile ? "34px 20px 48px" : "52px 32px 60px", display: "flex", flexDirection: "column", alignItems: "center" }}>
        {scanError && (
          <div role="alert" style={{ width: "100%", maxWidth: 580, display: "flex", alignItems: "flex-start", gap: 10, background: "#fdecea", border: "1px solid #f3b6ae", borderRadius: 12, padding: "12px 14px", marginBottom: 22, animation: "rise .4s both" }}>
            <span style={{ flex: 1, fontFamily: C.font, fontSize: 13.5, color: "#a13226", lineHeight: 1.5 }}>{scanError}</span>
            {onClearError && <button onClick={onClearError} aria-label="Dismiss" style={{ background: "none", border: "none", padding: 2, cursor: "pointer", display: "flex", flex: "0 0 auto" }}><IconClose s={16} c="#a13226" /></button>}
          </div>
        )}
